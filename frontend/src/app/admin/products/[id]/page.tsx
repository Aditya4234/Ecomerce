...
export default function EditProductPage() {
  ...
  const [colors, setColors] = useState(["#1e293b", "#6366f1", "#ec4899"]);
  const [tags, setTags] = useState(["wireless", "bluetooth", "premium"]);
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    ...
                    />
                    <Button type="button" variant="outline" size="sm" onClick={() => addItem(sizes, setSizes, sizeInput, setSizeInput)}>
                      Add
                    </Button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {sizes.map((s) => (
                      <span key={s} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-secondary text-sm">
                        {s}
                        <button type="button" onClick={() => setSizes(sizes.filter((x) => x !== s))} className="text-muted-foreground hover:text-foreground">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    ...
                    />
                    <Button type="button" variant="outline" size="sm" onClick={() => addItem(colors, setColors, colorInput, setColorInput)}>
                      Add
                    </Button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {colors.map((c) => (
                      <span key={c} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-secondary text-sm">
                        <span className="w-4 h-4 rounded-full border" style={{ backgroundColor: c }} />
                        {c}
                        <button type="button" onClick={() => setColors(colors.filter((x) => x !== c))} className="text-muted-foreground hover:text-foreground">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    ...
                    />
                    <Button type="button" variant="outline" size="sm" onClick={() => addItem(tags, setTags, tagInput, setTagInput)}>
                      Add
                    </Button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {tags.map((t) => (
                      <span key={t} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-secondary text-sm">
                        {t}
          ...
          <div className="space-y-6">
            ...
            <Button type="submit" className="w-full h-12" loading={saving}>
